...
export const IcicleAssistant: React.FC = () => {
    ...
    return (
        <div className="bg-background-secondary rounded-lg border border-border-color shadow-lg h-[30rem] flex flex-col">
            <div className="p-4 border-b border-border-color flex items-center space-x-3">
                ...
                <h3 className="text-lg font-bold text-text-primary">Icicle Assistant</h3>
            </div>
            <div className="flex-1 p-4 overflow-y-auto space-y-4">
                {messages.map((msg, index) => (
                    <div key={index} className={`flex items-end gap-2 ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                        {msg.sender === 'icicle' && <div className="w-6 h-6 bg-primary rounded-full flex-shrink-0"></div>}
                        <div className={`max-w-xs md:max-w-md px-4 py-2 rounded-lg ${msg.sender === 'user' ? 'bg-primary text-text-on-primary rounded-br-none' : 'bg-background-tertiary text-text-primary rounded-bl-none'}`}>
